...
impl Dist {
    ...
    pub fn sample(&self, rng: &mut impl Rng) -> Value {
        match self {
            Dist::Uniform { low, high } => {
                json_f64(rng.gen_range(*low..high.max(*low + f64::EPSILON)))
            }
            Dist::LogUniform { low, high } => {
                let (ll, lh) = (
...
impl SearchSpace {
    ...
    pub fn parse_param(spec: &str) -> Result<(String, Dist), String> {
        ...
        let open = rest
            .find('(')
            .ok_or_else(|| format!("--param '{spec}': missing '('"))?;
        if !rest.ends_with(')') {
        ...
        let want = |n: usize| -> Result<(), String> {
            if args.len() == n {
                ...
                Err(format!(
                    "--param '{spec}': {func} expects {n} args, got {}",
                    args.len()
                ))
            }
        };
...
mod tests {
    ...
    fn inline_param_parse() {
        ...
        assert_eq!(
            ...
            (
                "opt".into(),
                Dist::Choice {
                    choices: vec![Value::from("soap"), Value::from("adamw")]
                }
            )
        );
        ...
        assert_eq!(
            ...
            Dist::Choice {
                choices: vec![Value::from(2), Value::from(3)]
            }
        );
        ...
        assert!(
            SearchSpace::parse_param("x=uniform(1,2,3)").is_err(),
            "extra arg rejected"
        );
        assert!(
            SearchSpace::parse_param("x=quniform(1,10)").is_err(),
            "missing q rejected"
        );
        assert!(
            SearchSpace::parse_param("x=choice()").is_err(),
            "empty choice rejected"
        );
    }
    ...
    fn sampling_is_in_range_and_seed_reproducible() {
        ...
        s.dims
            .insert("bs".into(), Dist::IntUniform { low: 8, high: 16 });
        s.dims.insert(
            "q".into(),
            Dist::QUniform {
                low: 0.0,
    ...
    fn int_param_rejects_float() {
        assert!(
            SearchSpace::parse_param("x=int(1.9,3)").is_err(),
            "float arg to int() rejected"
        );
