...
# Inline NormConv2d (originally from encodec.modules.conv)
class NormConv2d(nn.Module):
    """Conv2d with LayerNorm — EnCodec found this is the only normalization
    that works reliably for STFT discriminators."""
    def __init__(self, *args, norm='layer_norm', **kwargs):
        super().__init__()
        # Strip 'norm' kwarg before passing to Conv2d
        self.conv = nn.Conv2d(*args, **kwargs)
        self.norm = nn.LayerNorm(self.conv.out_channels)
    def forward(self, x):
        x = self.conv(x)
        x = x.permute(0, 2, 3, 1)
        x = self.norm(x)
