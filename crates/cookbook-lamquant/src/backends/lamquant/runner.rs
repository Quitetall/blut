...
impl LamquantBackend {
    ...
    pub async fn run(
        ...
        *self.contained_unit.lock() = unit;
        if let Some(pid) = child.id() {
            *self.child_pid.lock() = Some(pid);
            ...
            blut::python_kill::set_active_child(blut::python_kill::capture_identity(pid));
        }
        ...
        *self.contained_unit.lock() = None;
        blut::python_kill::clear_active_child();
...
// ── Backend identity (folded marker) ──────────────────────────────
//
// Pre-C2a, blut-core held a SEPARATE marker unit-struct also named
// `LamquantBackend` carrying the `TrainingBackend` identity that the
// recipe DEFs declare as `type Backend` and every `Compatible<...>`
// targets. C2a moves both the marker and this runner into the
// cookbook; the collision is resolved by folding — this one runner
// struct plays BOTH roles. `ID`/`DESCRIPTION` are reproduced verbatim
// from the old marker (recipe DEFs read `<Backend>::ID`, so the wire
// identifier must not drift).
impl blut::backends::TrainingBackend for LamquantBackend {
