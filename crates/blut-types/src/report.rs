...
impl ProvenanceGraph {
    ...
    pub fn to_dot(&self) -> String {
        ...
        for n in &self.nodes {
            ...
            // Escape the node ID too, not just the label: `content_hash` is a
            // `String` on a `pub` type deserialized from the lineage DB / a
            // sidecar, so an unescaped `"` in the ID would close the quoted node
            // and inject DOT — same defense-in-depth as the label.
            let _ = writeln!(
                s,
                "  \"{}\" [label=\"{label}\"{shape}];",
                Self::dot_escape(&n.content_hash)
            );
        }
...
mod tests {
    ...
    #[test]
    fn dot_node_id_and_edges_escape_injection() {
        // `content_hash` is a `String` on a `pub`, deserialized type — a `"` in an
        // ID (node or edge endpoint) must be escaped, not just the label.
        let evil = "aa\" ]; hacknode [x=\"y".to_string();
        let g = ProvenanceGraph {
            root: evil.clone(),
            nodes: vec![
                GraphNode {
                    content_hash: evil.clone(),
                    stage_name: None,
                    kind: None,
                    job_id: None,
                },
            ...
            edges: vec![(evil.clone(), "bb".into())],
        };
        let dot = g.to_dot();
        // The raw unescaped quote sequence must never appear verbatim.
        assert!(
            !dot.contains("aa\" ]; hacknode"),
            "node ID quote must be escaped: {dot}"
        );
        assert!(
            dot.contains("aa\\\" ]; hacknode"),
            "escaped form present: {dot}"
        );
        // Structure intact: exactly two node lines and one edge line.
