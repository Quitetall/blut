...
>
> This is a working document from 2026-05-28, retained for provenance. Every
> finding, verdict, and count below describes a tree that no longer exists.
> Do not read any statement here as a claim about BLUT today. In particular:
>
> - Its headline verdict ("Does BLUT work today? NO") is a May assessment of a
>   May tree, against a bar this document defines for itself.
> - It describes a single crate at `v0.1.0` on rustc 1.85. The engine is now a
>   lib-only crate at `0.2.0-alpha.1` with an MSRV of 1.88 and nine sibling
>   crates.
> - Its premise is "**LamQuant-first**". That was superseded by ADR 0034: the
>   engine is domain-agnostic and ships no domain code. LamQuant is one
>   *cookbook* that depends on the engine.
>
> For current capability boundaries see `docs/DISTRIBUTED_VALIDATION.md`, which
> grades each claim as Validated / Component-tested / Deferred.
...
> `$DATA_ROOT/derived/lma` corpus all verified present.
...
| RCP-6 | high | recipes | `convert_lma` labels-dir default points at dead monorepo path (3 inconsistent label paths) | `lamquant_convert_lma.rs:142-143`; `bulk_lml_to_lma.py:66` | Default to `$DATA_ROOT/derived/labels`; unify across stage + script. |
...
| `convert_lma` idempotent-skip on live corpus | point at `$DATA_ROOT/derived/lma`, assert LmaCorpus emitted | **[ZERO]** (RCP-2) |
...
9. **RCP-6** — unify the labels-dir default to `$DATA_ROOT/derived/labels`.
